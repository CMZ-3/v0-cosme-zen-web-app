...
export function ImportPdfDialog({ open, onOpenChange, onImportComplete }: ImportPdfDialogProps) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Build extracted fields summary -- differs for JK vs JR
      const fields: ExtractedField[] = []
      const isJr = data.regType === "jr"
      fields.push({ key: "regType", label: "ประเภทเอกสาร", value: isJr ? "จ.ร.๑ (ใบรับจดแจ้ง)" : "จ.ค.๑ (คำขอจดแจ้ง)" })

      if (isJr) {
        // JR-specific fields
        if (data.regNumber) fields.push({ key: "regNumber", label: "เลขที่ใบรับจดแจ้ง", value: data.regNumber })
        if (data.issueDate) fields.push({ key: "issueDate", label: "วันที่ออก", value: data.issueDate })
        if (data.expiryDate) fields.push({ key: "expiryDate", label: "ใช้ได้จนถึง", value: data.expiryDate })
        if (data.issuedBy) fields.push({ key: "issuedBy", label: "ออกโดย", value: data.issuedBy })
        if (data.productNameTh) fields.push({ key: "productNameTh", label: "ชื่อ (ไทย)", value: data.productNameTh })
        if (data.productNameEn) fields.push({ key: "productNameEn", label: "ชื่อ (EN)", value: data.productNameEn })
        if (data.productNameSuffix) fields.push({ key: "productNameSuffix", label: "ชื่อแนบท้าย", value: data.productNameSuffix })
        if (data.cosmeticType) fields.push({ key: "cosmeticType", label: "ประเภทเครื่องสำอาง", value: data.cosmeticType })
        ...
        if (data.productFormat) fields.push({ key: "productFormat", label: "รูปแบบผลิตภัณฑ์", value: data.productFormat })
        if (data.businessType) fields.push({ key: "businessType", label: "ผู้ประกอบการ", value: { manufacture_sell: "ผลิตเพื่อขาย", contract_manufacture: "รับจ้างผลิต", import_sell: "นำเข้าเพื่อขาย" }[data.businessType] || data.businessType })
        if (data.cm_contractorName) fields.push({ key: "cm_contractorName", label: "ผู้รับจ้างผลิต", value: data.cm_contractorName })
        if (data.cm_factoryAddress) fields.push({ key: "cm_factoryAddress", label: "สถานที่ผลิต", value: data.cm_factoryAddress })
        if (data.cm_storageAddress) fields.push({ key: "cm_storageAddress", label: "สถานที่เก็บ", value: data.cm_storageAddress })
        ...
        if (data.ms_manufacturerName) fields.push({ key: "ms_manufacturerName", label: "ผู้ผลิต", value: data.ms_manufacturerName })
        if (data.imp_importerName) fields.push({ key: "imp_importerName", label: "ผู้นำเข้า", value: data.imp_importerName })
        if (data.bulkRegNo) fields.push({ key: "bulkRegNo", label: "เลขที่ Bulk (แบ่งบรรจุ)", value: data.bulkRegNo })
        if (data.combinedRegNos) fields.push({ key: "combinedRegNos", label: "เลขที่ (รวมบรรจุ)", value: data.combinedRegNos })
      } else {
        // JK-specific fields
        ...
        if (data.physicalForm) fields.push({ key: "physicalForm", label: "ลักษณะทางกายภาพ", value: data.physicalForm })
        if (data.containerType) fields.push({ key: "containerType", label: "ภาชนะบรรจุ", value: data.containerType })
        if (data.productFormat) fields.push({ key: "productFormat", label: "รูปแบบผลิตภัณฑ์", value: { single: "ผลิตภัณฑ์เดี่ยว", single_color_scent: "ต่างสี/กลิ่น", multi_combined: "รวมบรรจุ", set: "ชุดผลิตภัณฑ์" }[data.productFormat] || data.productFormat || "" })
        if (data.businessType) fields.push({ key: "businessType", label: "ผู้ประกอบการ", value: { manufacture_sell: "ผลิตเพื่อขาย", contract_manufacture: "รับจ้างผลิต", import_sell: "นำเข้าเพื่อขาย" }[data.businessType] || data.businessType })
        if (data.cm_contractorName) fields.push({ key: "cm_contractorName", label: "ผู้รับจ้างผลิต", value: data.cm_contractorName })
        if (data.cm_clientName) fields.push({ key: "cm_clientName", label: "ผู้ว่าจ้างผลิต", value: data.cm_clientName })
        if (data.cm_factoryAddress) fields.push({ key: "cm_factoryAddress", label: "สถานที่ผลิต", value: data.cm_factoryAddress })
